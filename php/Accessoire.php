<?php
   ...
   class Accessoire extends ROOT_Controller{
	 ...
	 function  creer($nom, $description)
   	 ...
   	 function  supprimer($id){
	 	  $requete = 'DELETE FROM `gv_Accessoire` WHERE `gv_Accessoire`.`id` = '.$id;
                  mysql_query($requete) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
   	 }
  	 ...
  	 function modifier($nom,$desc,$id){
		$requete = "UPDATE `gv_Accessoire` SET `nom`= '$nom' ,`description`= '$desc' WHERE `id`=$id";    
		mysql_query($requete) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
   	 }
   }
?>
